...
class FirewallController {
    ...
    getFirewallRules;
    deleteFirewallRules;
    updateFirewallRulesStatus;
    constructor(addFirewallRules, getFirewallRules, deleteFirewallRules, updateFirewallRulesStatus) {
        this.addFirewallRules = addFirewallRules;
        this.getFirewallRules = getFirewallRules;
        this.deleteFirewallRules = deleteFirewallRules;
        this.updateFirewallRulesStatus = updateFirewallRulesStatus;
    }
    async addIps(req, res) {
        const { values, mode } = req.body;
        const createdRules = await this.addFirewallRules(values, "ip", mode);
    ...
    async addDomains(req, res) {
        const { values, mode } = req.body;
        const createdRules = await this.addFirewallRules(values, "domain", mode);
        const responseValues = createdRules.map(rule => ({
            id: rule.id,
            value: rule.value,
            active: rule.active
    ...
    async addPorts(req, res) {
        const { values, mode } = req.body;
        const createdRules = await this.addFirewallRules(values, "port", mode);
        const responseValues = createdRules.map(rule => ({
            id: rule.id,
            value: rule.value,
            active: rule.active
    ...
    async getAllRules(req, res) {
        ...
        const filteredRules = type === undefined
            ? allRules
    ...
    async updateRulesStatus(req, res) {
        try {
            const { ids, active } = req.body;
            const updatedRules = await this.updateFirewallRulesStatus(ids, active);
            return res.status(200).json({
...
//# sourceMappingURL=firewallController.js.map